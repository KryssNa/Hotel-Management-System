...
conn=sqlite3.connect('billdash.db')
c=conn.cursor()
try:
    conn=sqlite3.connect("billdash.db")
    c=conn.cursor()
...
#====================Backend Functions===========================
    ...
    def load_item_from_menu(event):
        ...
        row = contents["values"]
#        itemCategory.set(row[2]) look on changes
        itemName.set(row[0])
        itemRate.set(row[1])
    ...
    combo_menu = ttk.Combobox(menu_category_frame,
                                textvariable=menuCategory)
    combo_menu['value']=(menu_category)
    ...
    # menu_tabel.insert('',END,values=["samosa","50"])
    ...
    # update_button =Button(item_frame3, text="Update Quantity",bg="#FFA726",command=update_button_operation,activebackground="#FFA726",
    #                     )
